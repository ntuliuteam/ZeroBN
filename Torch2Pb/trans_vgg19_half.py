...
def classifier(x, num_classes):
    ...
    w_fc = tf.Variable(tf.constant(Linear_weight[0], dtype=tf.float16))
    b_fc = tf.Variable(tf.constant(Linear_bias[0], dtype=tf.float16))
    x = tf.nn.bias_add(tf.matmul(x, w_fc), b_fc)
    x = tf.nn.relu(x)

    w_fc = tf.Variable(tf.constant(Linear_weight[1], dtype=tf.float16))
    b_fc = tf.Variable(tf.constant(Linear_bias[1], dtype=tf.float16))
    x = tf.nn.bias_add(tf.matmul(x, w_fc), b_fc)
    x = tf.nn.relu(x)

    w_fc = tf.Variable(tf.constant(Linear_weight[2], dtype=tf.float16))
    b_fc = tf.Variable(tf.constant(Linear_bias[2], dtype=tf.float16))
...
def make_layers(cfg, x, batch_norm=False):
    ...
    for v in cfg:
        if v == 'M':
            ...
            if batch_norm:
                w_conv = tf.Variable(tf.constant(Conv_weight[cur], dtype=tf.float16))
                b_conv = tf.Variable(tf.constant(Conv_bias[cur], dtype=tf.float16))
                x = tf.nn.bias_add(conv2d(x, w_conv), b_conv)

                mean = tf.Variable(tf.constant(BN_mean[cur], dtype=tf.float16))
                var = tf.Variable(tf.constant(BN_var[cur], dtype=tf.float16))
                scale = tf.Variable(tf.constant(BN_weight[cur], dtype=tf.float16))
                shift = tf.Variable(tf.constant(BN_bias[cur], dtype=tf.float16))
                epsilon = 0.00001
...
def main():
    ...
    x_input = tf.placeholder(dtype=tf.float16, shape=[None, 224, 224, 3], name="Input")
    vgg = vgg19()
...
if __name__ == '__main__':
    ...
    for m in model.modules():
        if isinstance(m, nn.Linear):
            if ll == 0:
                process = m.weight.data.half().cpu().numpy().reshape(4096, 512, 7, 7)
                process = torch.tensor(process).transpose(0, 1).numpy()
                process = process[last_nonzero]
                process = torch.tensor(process).transpose(0, 1).half().numpy()
                process = process.reshape(4096, last_layer * 7 * 7)
                process = torch.tensor(process).transpose(0, 1).half().numpy()

                Linear_weight.append(process)
                Linear_bias.append(m.bias.data.half().cpu().numpy())

                ll = ll + 1

            else:
                Linear_weight.append(m.weight.data.half().transpose(0, 1).cpu().numpy())
                Linear_bias.append(m.bias.data.half().cpu().numpy())

        if isinstance(m, nn.Conv2d):
            Conv_weight.append(m.weight.data.half().cpu().numpy())
            Conv_bias.append(m.bias.data.half().cpu().numpy())

        if isinstance(m, nn.BatchNorm2d):
            weight = m.weight.data.half().cpu().numpy()
            bias = m.bias.data.half().cpu().numpy()
            ...
            BN_mean.append(m.running_mean.data.half().cpu().numpy()[nonzero_index])
            BN_var.append(m.running_var.data.half().cpu().numpy()[nonzero_index])
            ...
            if last_nonzero is not None:
                ...
                process = Conv_weight[conv_last][nonzero_index]
                process = torch.tensor(process)
                process = process.transpose(0, 3).transpose(1, 2).transpose(0, 1).half().numpy()
                Conv_weight[conv_last] = process
