...
class BasicConv2d:
    ...
    def forward(self, x):
        ...
        w_conv = tf.Variable(tf.constant(Conv_weight[conv_weight_trans], dtype=tf.float16))
        ...
        mean = tf.Variable(tf.constant(BN_mean[bn_weight_trnas], dtype=tf.float16))
        var = tf.Variable(tf.constant(BN_var[bn_weight_trnas], dtype=tf.float16))
        scale = tf.Variable(tf.constant(BN_weight[bn_weight_trnas], dtype=tf.float16))
        shift = tf.Variable(tf.constant(BN_bias[bn_weight_trnas], dtype=tf.float16))
        epsilon = 0.001
